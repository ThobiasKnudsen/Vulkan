...
Gui_GraphicsPipeline Gui_GraphicsPipeline_Initialize(
    Vk* p_vk) 
{
    VERIFY(p_vk, "NULL pointer passed to Gui_GraphicsPipeline_Create");
    Gui_GraphicsPipeline rendering;
    memset(&rendering, 0, sizeof(Gui_GraphicsPipeline));
    rendering.p_vk = p_vk; 
    return rendering;
}

void Gui_GraphicsPipeline_AddShader(
    Gui_GraphicsPipeline* p_pipeline, 
    SpvShader spv_shader, 
    shaderc_shader_kind shader_kind) 
{
    VERIFY(p_pipeline, "NULL pointer passed to Gui_GraphicsPipeline_AddShader");
    ...
    for (unsigned int i = 0; i < p_pipeline->shaders_count; ++i) {
        VERIFY(shader_kind != p_pipeline->p_shaders[i].shader_kind, 
               "A shader of the given shader_kind already exists in this Gui_GraphicsPipeline instance");
    }
...
void Gui_GraphicsPipeline_CreatePipeline(
    Gui_GraphicsPipeline* p_pipeline) 
{

    VERIFY(p_pipeline, "NULL pointer passed to Gui_GraphicsPipeline_CreateDescriptorSets");
